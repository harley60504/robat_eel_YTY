...
def main():
    ...
    # Keep the oscillating CPG wave identical to the Release/Flutter backend.
    # The XML joint axes are already flipped to axis="0 0 -1" so the CPG swing
    # direction matches the physical servo direction. However, the simulated
    # hydrodynamic turning response is mirrored, so only the static steering
    # bias is inverted here.
    mujoco_joint_bias = tuple(-float(value) for value in gait["joint_bias"])
    ...
    print("  json_joint_bias=", ", ".join(f"{float(value):.3f}" for value in gait["joint_bias"]))
    print("  mujoco_joint_bias=", ", ".join(f"{value:.3f}" for value in cpg_params.joint_bias or ()))
    print("  MuJoCo adapter: servo joint axes are axis=\"0 0 -1\"; steering bias is inverted")
